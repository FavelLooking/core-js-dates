...
function getNextFriday(date) {
  ...
  const todayDay = realDate.getDay();

  const daysToFriday = (5 - todayDay + 7) % 7;
  let finalDate = realDate.getTime() + 86400000 * daysToFriday;
  if (todayDay === 5) {
    finalDate = realDate.getTime() + 86400000 * 7;
  }
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const time = endOfPeriod.getTime() - startOfPeriod.getTime();
  const totalDays = Math.ceil(time / (1000 * 60 * 60 * 24)) + 1;
  return totalDays;
}
...
function isDateInPeriod(date, period) {
  const startOfPeriod = new Date(period.start);
  const endOfPeriod = new Date(period.end);
...
function formatDate(date) {
  ...
  let hour = currentDate.getUTCHours() % 12;
  let timeOfDay;
  ...
  const finalString =
    `${month}/${day}/${year}, ${hour}:${minute}:${second} ${timeOfDay}`.trim();
  return finalString;
}
...
function getWeekNumberByDate(date) {
  ...
  const januaryFirstDayOfWeek = newDate.getDay();

  let notFullWeekDays;
  if (januaryFirstDayOfWeek === 0) {
    notFullWeekDays = 6;
  } else {
    notFullWeekDays = januaryFirstDayOfWeek - 1;
  }

  newDate.setDate(newDate.getDate() - notFullWeekDays);
